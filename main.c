...
// stores the range of grades the user predicts theyll get. for the forecast
// mode
...
// allows users to input a course and writes that into the file containing the
// database
void inputCourse(FILE *fp) {
    ...
    while (1) {
        for (;;) {
            ...
            if (read != 3) {
                printf(
                    "Error. Incorrect format. Please try again and input what is requested in the proper format.\n\n"
                );
                continue;
            }
            ...
            if (temp < 1908) {
                printf(
                    "Error. The inputted academic year was before UP was established. Please try again.\n\n"
                );
                continue;
            }
            data = strtok(NULL, " ");
            ...
            if ((temp2 - temp) != 1) {
                printf(
                    "Error. The inputted years are more than 1 year apart. Please try again.\n\n"
                );
                continue;
            }
            ...
            if (!(codeToUnits(c.code))) {
                printf(
                    "Error. The inputted course code isn't in the database. Please try again.\n\n"
                );
                continue;
            }
        ...
        for (;;) {
            ...
            if (flush_stdin() || read < 1 || (ans != 1 && ans != 0)) {
                printf(
                    "Error! Invalid input. Inputs must either be 1 for yes or 0 for no. Try again.\n\n"
                );
                continue;
            }
            break;
        ...
        if (ans) {
            for (;;) {
                ...
                if (c.grade > 3.00 && c.grade < 5.00) {
                    printf(
                        "Error! Invalid input. A grade in the range (3.00,5.00) does not exist in the UP grading system.\nA grade of 4.0 does exist, however, it is a temporary classification.\n\n"
                    );
                    continue;
                }

                if ((c.grade < 1.00 || c.grade > 5.00) ||
                    !(isValidGrade(c.grade))) {
                    printf(
...
// allows users to edit the file to add in the grades of classes taken in a
// specific semester of a specific acad year
void inputGrade(FILE *fp) {
    ...
    for (;;) {
        printf(
            "Enter the academic school year and semester you wish to input grades for:\n"
        );
        while (fgets(buffer, sizeof(buffer), stdin) && buffer[0] == '\n')
            ;
    ...
    printf(
        " courses inputted that occur within the school year and semester you inputted.\n\n"
        "Here is the updated database.\n\n"
    );
    printFile(fp);
    return;
...
/* takes the inputted acad year and semester, scours the entire file for all the
courses which were taken during the provided time and computes for the gwa and
returns the result
*/
...
// takes an academic year and semester from the user, calculates the GWA for
// that semester and prints the result
void calcuGWA(FILE *fp) {
    ...
    for (;;) {
        printf(
            "Enter the academic school year and semester you wish to calculate the GWA for:\n"
        );
        while (fgets(buffer, sizeof(buffer), stdin) && buffer[0] == '\n')
            ;
        read = sscanf(buffer, "%s %d", ay, &sems);
...
// does the necessary calculations to determine what grades the user must
// acquired given the set parameters
void forecaster(
    char *ay, int sems, float gwa, courseProfile *arr, int size,
    minmaxGrade *range
) {
    float target, tempGWA = 0, bestgradeGWA, worstgradeGWA;
    int sumUnits = 0, units = 5, running_heavy = 0, running_light = 0, max = 0,
        preset_units = 0;
    int unit_count[6] = {0};
    ...
    if (minGWA < 1) {
        printf(
            "It is impossible to attain your target GWA with the parameters set.\nKindly try again"
        );
        return;
    }
    ...
    printf(
        "semester of the %s A.Y. and the inputted paramters,\none must achieve the following grades:",
        ay
    );
    ...
    for (int i = 0; i < 6; i++) {
        if (unit_count[i] != 0 && running[i] == 0) {
            printf(
                "\n For courses with %d units, one must have at the minimum, an average grade of %.2f.",
                i, rem_coursegrade[i]
            );
            ...
            if (i == min && min != max) {
                ...
                printf(
                    "\n For courses with %d units, one must have at the at the minimum, an average grade of %.2f.",
                    i, range->worst
                );
            } else {
                printf(
                    "\n For courses with %d units, one must have a grade of %.2f at least %d times and a grade\n of %.2f at least %d times.",
                    i, range->worst, running[i], range->best,
                    running[i] - range->numWorst
                );
            }
        } else if (running[i] == 0 && unit_count[i] == 0) {
            continue;
            ...
            if (i != max && max != min) {
                printf(
                    "\n For courses with %d units, one must have a grade of %.2f at least %d times and %.2f for every other\ncourse with the same amount of units.",
                    i, range->best, running[i], rem_coursegrade[i]
                );
            } else if (i == max && max != min) {
                printf(
                    "\n For courses with %d units, one must have a grade of %.2f at least %d times and %.2f for every other\ncourse with the same amount of units.",
                    i, range->worst, running[i], rem_coursegrade[i]
                );
            } else {
                printf(
                    "\n For courses with %d units, one must have a grade of %.2f at least %d times, a grade\n of %.2f at least %d times and an average grade of %.2f for every other course with the same amount of units.",
                    i, range->worst, running[i], range->best,
                    running[i] - range->numWorst, rem_coursegrade[i]
                );
            }
        }
    }
...
// menu for gwa forecasting. allows users to set parameters in order to forecast
// their gwa
void gwaForecast(FILE *fp) {
    ...
    for (;;) {
        printf(
            "Enter the academic school year and semester you wish to forecast the GWA for:\n"
        );
        while (fgets(buffer, sizeof(buffer), stdin) && buffer[0] == '\n')
            ;
        read = sscanf(buffer, "%s %d", ay, &sems);
    ...
    for (;;) {
        ...
        if (flush_stdin() || read < 1 || fGWA < 1) {
            printf(
                "Error! Invalid input. Inputs must be a number between 1.00 and 3.00. Try again.\n\n"
            );
            continue;
        }
        ...
        if (fGWA > 3.00 && fGWA < 5.00) {
            printf(
                "A GWA lesser than 3.00 means failing several subjects. Aim higher.\n\n"
            );
            continue;
        }
        ...
        if (fGWA == 1.00) {
            printf(
                "In order to get a GWA of 1.00, you will have to get a grade of 1.00 in every course you are taking\nin that semester.\n\n"
            );
            return;
        }
        break;
    ...
    do {
        ...
        case 1:
            do {
                ...
                for (int i = 0; i < num; i++) {
                    printf(
                        "%d %s %d %s %d %0.2f\n", i + 1, arr[i].aYear,
                        arr[i].sem, arr[i].code, arr[i].units, arr[i].grade
                    );
                }
                for (;;) {
                    printf(
        ...
        case 2:
            for (;;) {
                ...
                if ((range.best < 1.00 || range.best > 5.00) ||
                    (((int)(100 * range.best)) % 25) != 0) {
                    printf(
                        "\nError! Invalid input. The UP grading system uses numbers in 0.25 increments. Try again\n\n"
                    );
                    continue;
            ...
            for (;;) {
                ...
                if ((range.worst < 1.00 || range.worst > 5.00) ||
                    (((int)(100 * range.worst)) % 25) != 0) {
                    printf(
                        "\nError! Invalid input. The UP grading system uses numbers in 0.25 increments. Try again\n\n"
                    );
                    continue;
            ...
            for (;;) {
                ...
                if (flush_stdin() || read < 1 ||
                    range.numWorst > (num - range.numBest)) {
                    printf(
                        "\nError! Invalid input. Inputs must be a number between 1 and %d. Try again.\n",
                        num - range.numBest
                    );
